...
def training_pipeline(path, model_info_db_name='models-db-laura'):
    """
...
def save_model(obj, name, timestamp, bucket_name='models-uem-laura'):
    """
...
def get_best_model(model_metrics1, model_metrics2):
    ...
    # comparación de modelos usando la métrica accuracy score.
    acc1 = model_metrics1['model_metrics']['accuracy_score']
    acc2 = model_metrics2['model_metrics']['accuracy_score']
    print('------> Model comparison:')
    print('---------> TRAINED model {} with accuracy score: {}'.format(model_metrics1['_id'], str(round(acc1, 3))))
    print('---------> CURRENT model in PROD {} with accuracy score: {}'.format(model_metrics2['_id'], str(round(acc2, 3))))
